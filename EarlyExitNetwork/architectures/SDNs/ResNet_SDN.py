...
import EENNO.EarlyExitNetwork.aux_funcs as af
import EENNO.EarlyExitNetwork.model_funcs as mf
...
class BasicBlockWOutput(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.layers[2](fwd), 1, self.output(fwd)
...
class ResNet_SDN(nn.Module):
    ...
    def early_exit(self, x):
        ...
        max_confidence_output = torch.argmax(confidences_tensor).item()
        is_early = False
