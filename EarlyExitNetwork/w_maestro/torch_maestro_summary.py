...
def summary(model, input_size, device="cuda:1", batch_size=-1, silent=True):
    ...
    if device == "cuda:1" and torch.cuda.is_available():
    ...
    x = tuple(input_tensor.to(device) for input_tensor in x)
    # make a forward pass
    model(*x)
