...
from architectures.thresholdselect.MobileNet_threshold import MobileNet_SDN
...
def create_vgg16bn(models_path, task, pos_exits, save_type, get_params=False):
...
def create_resnet56(models_path, task, pos_exits, save_type, get_params=False):
    model_params = get_task_params(task)
    ...
    return save_networks(model_name, model_params, models_path, save_type)  # save_type='cd'


def create_mobilenet(models_path, task, pos_exits, save_type, get_params=False):
...
def get_lr_params(model_params):
    ...
    model_params['milestones'] = [35, 60, 80]
...
def get_net_params(net_type, task, pos_exits):
    if net_type == 'vgg16':
        return create_vgg16bn(None, task,  pos_exits, None, True)
    elif net_type == 'resnet56':
        return create_resnet56(None, task, pos_exits, None, True)
    elif net_type == 'mobilenet':
        return create_mobilenet(None, task, pos_exits,  None, True)
