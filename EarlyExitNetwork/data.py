...
from TinyImageNet import TinyImageNet_load
...
class CIFAR10:
    def __init__(self, batch_size=128, add_trigger=False):
        ...
        self.aug_trainset = datasets.CIFAR10(root='./data', train=True, download=False, transform=self.augmented)
        self.aug_train_loader = torch.utils.data.DataLoader(self.aug_trainset, batch_size=batch_size, shuffle=True, num_workers=4)

        self.trainset = datasets.CIFAR10(root='./data', train=True, download=False, transform=self.normalized)
        self.train_loader = torch.utils.data.DataLoader(self.trainset, batch_size=batch_size, shuffle=True)

        self.testset = datasets.CIFAR10(root='./data', train=False, download=False, transform=self.normalized)
        ...
        if add_trigger: 
            ...
            self.trigger_test_set = datasets.CIFAR10(root='./data', train=False, download=False, transform=self.trigger_transform)
...
class CIFAR100:
    def __init__(self, batch_size=128):
        ...
        self.trainset =  datasets.CIFAR100(root='./data', train=True, download=False, transform=self.normalized)
        self.train_loader = torch.utils.data.DataLoader(self.trainset, batch_size=batch_size, shuffle=True, num_workers=4)

        self.testset =  datasets.CIFAR100(root='./data', train=False, download=False, transform=self.normalized)
        self.test_loader = torch.utils.data.DataLoader(self.testset, batch_size=batch_size, shuffle=False, num_workers=4)
...
class TinyImagenet():
    def __init__(self, batch_size=128):
        ...
        self.aug_train_loader = torch.utils.data.DataLoader(self.trainset, batch_size=batch_size, shuffle=True,
                                                            num_workers=4)

        data_dir = ''
        self.testset = TinyImageNet_load(data_dir, train=False, transform=self.normalized)
