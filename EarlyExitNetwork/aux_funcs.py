...
from brief_data import SimpleCIFAR10
...
def extend_lists(list1, list2, item1, item2):
    list1.append(item1)
    list2.append(item2)
...
def save_position_error_to_csv(position, threshold, average_mult_ops, error, exit_counts, non_conf_output_counts, filename='position_error.csv'):

    script_dir = os.path.dirname(os.path.abspath(__file__))  # Get the directory of the script
    file_path = os.path.join(script_dir, filename)


    fieldnames = ['position', 'threshold', 'average_mult_ops', 'portion', 'error', 'exit_counts', 'non_conf_output_counts']
    portion = average_mult_ops/0.252573
    with open(file_path, 'a', newline='') as csvfile:
        ...
        pos_content = {
            ...
            'threshold': threshold,
            'average_mult_ops': average_mult_ops,
            'portion': portion,
            'error': error,
            'exit_counts': exit_counts,
            'non_conf_output_counts':non_conf_output_counts
        }
        writer.writerow(pos_content)
